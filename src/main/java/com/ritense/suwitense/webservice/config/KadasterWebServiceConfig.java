...
@EnableWs
@Configuration
public class KadasterWebServiceConfig extends WsConfigurerAdapter {
    Logger logger = LoggerFactory.getLogger(KadasterEndpoint.class);

    @Value("${classpath:suwinet/KadasterDossierGSD/Diensten/KadasterDossierGSD/v0300-b02/Body*.xsd}")
    ...
    public void addInterceptors(List<EndpointInterceptor> interceptors) {
        logger.info("KadasterDossierGSD WebServiceConfig adding PayloadValidatingInterceptor");
        PayloadValidatingInterceptor validatingInterceptor = new PayloadValidatingInterceptor();
        validatingInterceptor.setValidateRequest(false);
        validatingInterceptor.setValidateResponse(false);
        try {
            validatingInterceptor.setXsdSchemaCollection(getKadasterDossierGSDXsds());
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Bean(name = "KadasterDossierGSD")
    public ServletRegistrationBean<MessageDispatcherServlet> messageDispatcherServlet(ApplicationContext applicationContext) {
        MessageDispatcherServlet servlet = new MessageDispatcherServlet();
        servlet.setApplicationContext(applicationContext);
        servlet.setTransformWsdlLocations(true);
        return new ServletRegistrationBean<>(servlet, "/ws/KadasterDossierGSD/*");
    }
}